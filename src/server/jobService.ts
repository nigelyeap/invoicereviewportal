...
/**
 * Orchestrates Document + ExtractionJob lifecycle. Route handlers AND the
 * worker both go through this file rather than touching Prisma directly --
 * see the project-structure rule in the plan doc.
 */
...
/**
 * Worker entrypoint: atomically claims the oldest PENDING job so multiple
 * worker processes can run safely (`FOR UPDATE SKIP LOCKED`), flipping it
 * to SUBMITTING. Returns null if there's nothing to do.
 */
